...
export async function buildContextMarkdown(): Promise<string> {
  ...
  lines.push("- **3 eras × 3 maps, no target creature count per map or era** (the `~20 unique per map` goal was dropped in 2026-09). A map's roster is whatever carries its `mapId`; the count is a query, not a rule. Reappearances on later maps are intentional and unrestricted.");
  lines.push("");
