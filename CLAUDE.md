...
- `ability_stats` — 1:1 com habilidade. `power` 0 = movimento de status; `effectCode` é o switch que o Godot roda. `attackVariant` (desde 2026-09) é um switch SEPARADO — decide qual clipe de corpo o golpe toca (`attack`/`attack2`/`attack3`), não o efeito mecânico. Hoje as 15 habilidades elementais são `attack2`, as 5 do Despertar são `attack3`, o resto (básicos de classe, buff, debuff, cura, carga) fica em `attack` (default; buff/debuff/cura/carga nunca tocam clipe de corpo, só efeito visual). Ver `avyron/ANIMACOES_PENDENTES.md` pro vocabulário completo do lado do jogo.
- `capture_rules` — 1:1 com criatura. `catchRate` 1–255.
- `creature_abilities` — junção: qual criatura sabe qual golpe, em que nível. **O PZ-01 segue um esquema padronizado desde 2026-09: exatamente 4 golpes por criatura** (2 de dano fora do Despertar — o básico da classe e o elemental da própria criatura —, 1 buff, 1 elemental exclusivo do Despertar). Guarda de código em `export-game-data.mjs:843-871` (espelhado em `avyron/scripts/dev/test_data.gd`); resto do elenco ainda segue o padrão antigo de 5-6 golpes.
...
pnpm models:publish -- --code CRT-XXX   # casca do Tripo → esqueleto da mestre → optimize → espelho no jogo (fluxo base + casca, ver seção abaixo)
pnpm models:optimize      # teto de textura 2048² e emissivo morto fora — o publish já roda; solto, é idempotente
pnpm models:tripo         # só a conversão (convert-tripo.mjs), sem publicar
pnpm species:prompt -- --code CRT-XXX   # pasta da espécie com prompt de desenho preenchido pelo bundle + gabarito da mestre
pnpm species:check -- --code CRT-XXX    # confere a folha 2×2 restilizada contra a mestre antes do Tripo Studio
...
**Corpo de criatura é "base + casca" desde 2026-09-15, e o Meshy saiu do caminho das criaturas.** Todo corpo do elenco divide a MESMA base: o esqueleto de 55 ossos da mestre (`../mestre/manequim-mestre.glb`, no diretório irmão `games/mestre/` — um manequim chibi liso vestido no esqueleto do Puglin do kit Dungeon Monsters, nomes de osso da UAL, T-pose, 0,93 m). Cada espécie traz só a casca: malha e textura geradas no Tripo Studio sobre a silhueta da mestre (folha 2×2 de quatro vistas ortográficas, restilizada num gerador de imagem pelo prompt que `species-prompt.mjs` preenche a partir do bundle), vestidas no esqueleto por `scripts/convert-tripo.mjs` — transferência de pesos por proximidade (K vizinhos da mestre, inverso da distância), com três regras que existem por medição e não por gosto: (a) **rig adaptativo** — o gerador de imagem devolve braço e perna 10–25% mais curtos que a mestre, então os ossos do braço, da perna e a altura do quadril são escalados à casca (faixa 0,6×–1,4×), as matrizes de bind recomputadas e a malha da mestre reposta por skinning antes da transferência; (b) **apêndices rígidos** — espinho, garra e cauda (trecho a mais de 4 cm da mestre que protrai 7 cm ou mais) recebem um peso só, a média da base onde encostam, com rampa entre 4 e 7 cm, porque pesos que variam ao longo do apêndice o rasgam quando os ossos se separam, e sem a rampa (ou sem soldar os vértices duplicados nas costuras de UV) a cauda se abre em dois; (c) **zero clipes no arquivo** — a animação é a biblioteca UAL, dada em runtime pelo jogo pelo mesmo caminho do placeholder Imp (`CreatureActor._build_retargeted_animation`), então nenhum gesto é transplantado de uma criatura para outra e nenhum clipe é bakeado. Resultado: `<CODE>.glb` solto em `apps/web/public/models/`, 1:1 com a criatura, `syncModels` liga o `modelUrl` pelo nome. O comando de ponta a ponta é `pnpm models:publish -- --code <CODE>` (`scripts/publish-shell.mjs`), e o checklist inteiro, com o que cada número impresso quer dizer, está em `../mestre/README.md`. Os corpos Meshy anteriores (rig próprio de 24 ossos com `Hips`, clipes bakeados, transplante de gestos entre corpos, remoção de root motion) foram substituídos nos 14 do PZ-01 em 2026-09-16/17; `convert-meshy.mjs` e `transfer-clips.mjs` foram apagados, e os arquivos Meshy (exports crus e corpos publicados) estão em `../shared-assets/legacy/` (diretório irmão `games/shared-assets/legacy/`, com README), fora dos dois repositórios. Não resta corpo Meshy em jogo: o jogador voltou ao kit de personagens em 2026-09-17 (receita `PlayerController.PLAYER_RECIPE`), e o `.glb` Meshy dele está em `../shared-assets/legacy/meshy-player/`.

**Sem KTX2 desde 2026-09-17.** O site não renderiza modelo desde 2026-09; o único consumidor é o Godot, que escurecia a cor ao decodificar ETC1S e comprime PNG/JPEG para VRAM sozinho na importação. O `models:optimize` codificava KTX2 só para o espelho do jogo decodificar de volta. Hoje o `optimize` só aplica teto de 2048² e tira emissivo morto, e `game:export` espelha o `.glb` por cópia direta — ver [docs/MODEL_OPTIMIZATION.md](docs/MODEL_OPTIMIZATION.md).
...
- **Não colocar corpo de criatura em `apps/web/public/models/` à mão.** O caminho é `pnpm models:publish -- --code <CODE>`: ele converte, normaliza, guarda o anterior e espelha. Um `.glb` copiado à mão chega sem esqueleto da mestre (o jogo cai no placeholder) ou com textura fora do teto. Ver [docs/MODEL_OPTIMIZATION.md](docs/MODEL_OPTIMIZATION.md).
- **Não reintroduzir KTX2 nos corpos.** Ninguém decodifica: o site não renderiza e o Godot escurece a cor ao decodificar ETC1S. O espelho do jogo é cópia direta desde 2026-09-17.
...
- **docs/MODEL_OPTIMIZATION.md** — o contrato do `.glb` de criatura (esqueleto da mestre, sem clipe, textura ≤ 2048², sem KTX2) e o que o `models:optimize` faz
- **`../mestre/README.md`** (diretório irmão `games/mestre/`) — o checklist do fluxo base + casca, espécie por espécie, e como ler os números do conversor
...
- **`scripts/convert-tripo.mjs`** — veste uma casca do Tripo no esqueleto da mestre: alinhamento, rig adaptativo, pesos por proximidade, apêndices rígidos
- **`scripts/publish-shell.mjs`** — `pnpm models:publish`: casca → `apps/web/public/models/<CODE>.glb` → optimize → espelho no jogo
- **`scripts/species-prompt.mjs`** / **`check-sheet.mjs`** — prompt de desenho por espécie a partir do bundle, e conferência da folha 2×2 contra a mestre antes do Tripo Studio
