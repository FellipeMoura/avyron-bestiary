...
- **Código = inglês:** tabelas (`creatures`, `creature_stats`, `creature_classes`, `game_maps`, `design_documents`), colunas (`original_name`, `awakening_multiplier`), endpoints (`/api/v1/creatures/{code}`), variáveis, componentes, pastas.
...
- **Enums no banco = inglês** (`paleozoic`, `defined`, `charge_gain`), traduzidos na apresentação via `labels.ts` (`ERA_LABEL`, `DOCUMENT_STATUS_LABEL`, `ABILITY_EFFECT_LABEL`).
- **Prefixos de código dos dados** preservados dos fontes originais em português: `CRT`, `ELE`, `CLS`, `BIO`, `HAB`, `ITM`, `NPC`, `MIS`, `DRP`. São valores, não código.
...
- Recursos com FK (creatures, missions, npcs, abilities) — Service manual porque precisa resolver códigos para ids dentro da transação via `resolveCodeInTx` (`apps/api/src/shared/services/fkResolver.ts`)
- `design_documents` é o único recurso com `DELETE /documents/{slug}` — adicionado em 2026-09 para apagar uma regra que deixou de existir (a 70/30 dos Despertares), porque documento descontinuado com aviso no corpo fica em `/docs` para sempre. Grava o changelog **antes** do delete, como `creaturesService.remove`
...
- **Despertar Ancestral é universal e é um buff, não uma transformação.** Desde 2026-09 não existe tabela por criatura (a `awakenings`, com nome próprio, tipo reforço/troca e espécie de referência, foi removida, e a regra 70/30 foi junto): toda criatura desperta, os dois lados do duelo despertam (a IA ativa sozinha), e os dois números — `awakeningMultiplier` sobre Ataque e Defesa, `awakeningDurationTurns` — moram em `combat_rules`. O que é da criatura é só o golpe `awakeningOnly` que o buff libera. O motivo da simplificação: o jogo nunca leu tipo, nome nem espécie de referência, e a cobertura 1:1 virou dívida de conteúdo permanente. Ver `despertar-ancestral` e `carga-e-despertar`.
- **O intervalo `CRT-001..014` é do PZ-01, e é exatamente o elenco com modelo 3D.** Renumeração de 2026-09: o PZ-01 caiu de 20 para as 14 espécies com corpo Meshy pronto (saíram Isotelus, Orthoceras, Cameroceras, Sacabambaspis, Cothurnocystis e Aegirocassis — `CRT-033/036/037/039/040/041`, apagadas com cascata), os quatro códigos do intervalo que não eram do PZ-01 foram empurrados para o fim (`CRT-008→068` Arthropleura, `009→069` Meganeura, `010→070` Pulmonoscorpius, `011→071` Rhyniognatha) e os sete do PZ-01 que estavam fora entraram nas vagas em ordem crescente (`015→006` Odaraia, `034→008` Marrella, `035→009` Leanchoilia, `038→010` Arandaspis, `042→011` Endoceras, `043→012` Echinosphaerites, `044→013` Megalograptus). `DSP-*` acompanhou (`DSP-NNN` = `CRT-NNN`). Documento ou medição anterior a essa data que cite um desses códigos fala da espécie antiga — o changelog 0.521–0.542 tem o mapa completo. Não existe regra automática que mantenha o intervalo: criatura nova do PZ-01 entra em `CRT-015+` normalmente.
- **3 eras, cada uma dividida em 3 submapas (9 mapas no total).** Cada submapa é um `game_map` próprio (`era` compartilhada, `code` sequencial: `PZ-01/02/03`, depois `MZ-*`, `CZ-*`) e representa um bloco cronológico/ecológico, não um único período geológico rígido. A macroprogressão de cada era é decisão de design; a de Aetheris é MAR → MARGEM → TERRA. **Não existe quantidade-alvo de criaturas por mapa nem por era** — a meta de `~20 inéditas por mapa` foi retirada em 2026-09. O elenco de um mapa é o que está cadastrado com `mapId` nele, e a contagem é consulta, não regra. Reaparições em mapas posteriores continuam intencionais e sem restrição de contagem. Quais mapas existem e em que ordem: `game_maps` (`GET /maps`); quais biomas cada um tem: `map_biomes`. Ver `eras-e-regioes`.
...
- `combat_rules` — **singleton** (`id = 1`, garantido por CHECK). As constantes de tuning: escala de dano, variância, taxas de enchimento da carga, multiplicador e duração do Despertar, limites de captura, teto de nível. `GET /combat-rules` e `PATCH /combat-rules` — sem código, sem lista, sem POST. **É aqui que se balanceia o jogo.**
...
**Carga do Despertar:** enche com dano recebido (×`chargeTakenMultiplier`) e causado (×`chargeDealtMultiplier`), escalado por `baseCharge / chargeNeutralCharge`. Cheia em `chargeMax`; ativar **zera o medidor** (não a reversão), não consome o turno, e a IA adversária ativa sozinha. Enquanto ativo: Ataque e Defesa × `awakeningMultiplier` por `awakeningDurationTurns` rodadas, golpes `awakeningOnly` liberados; trocar de criatura reverte. Receber enche mais que causar — deliberado, para o Despertar ser virada de jogo e não amplificador de vitória.
...
pnpm models:placeholders  # converte placeholder_models/ (.gltf OU .glb) em .glb com clipes normalizados
...
pnpm models:dungeon       # copia corpos "Dungeon Monsters" (Quaternius) sem clipe — o jogo faz retarget na UAL em runtime
pnpm models:meshy         # funde export multi-arquivo do Meshy AI (um .glb por clipe) num .glb multi-clipe só — ver seção abaixo
pnpm models:transfer      # transplanta clipes de um corpo Meshy pra outro do mesmo rig (retarget relativo ao repouso) — ver seção abaixo
pnpm models:strip         # remove root motion in-place de .glb já convertidos (os de antes de 2026-09-07 chegaram andando) — ver seção abaixo
pnpm models:materials     # corpo Meshy → PBR comum (metallic 0, sem emissivo-como-cor) — o models:meshy já faz isso em corpo novo; este é pro elenco existente
...
**O export tem dois níveis.** Contradição de dado **aborta** (criatura sem stats/captura/golpes, taxa de mineração apontando para item não-mineral, classe do elenco sem pesos ou sem `workFunction`, `modelUrl` quebrado, loja vazia…); meta de conteúdo **avisa e escreve** (elemento sem paleta, bioma de mapa sem `mining_rates`). O mesmo par de critérios vale em `scripts/dev/test_data.gd` no repo do jogo — os dois guardas precisam concordar sobre o que é erro e o que é alvo. Lista completa em [docs/DATA_WORKFLOW.md](docs/DATA_WORKFLOW.md). `pnpm game:export` aceita `--from <url>` (default API local) e `--out <path>` (default `../avyron`). Ele **falha** se alguma criatura estiver sem stats, sem regra de captura ou sem golpes — em vez de gerar um bundle que quebra o jogo em runtime. Além do `data/bestiary.json`, ele espelha todo `.glb` referenciado por `modelUrl` em `<repo-godot>/models/`, preservando o caminho da URL (o jogo lê `res://models/...`); `modelUrl` apontando para arquivo inexistente também aborta o export. Os props de bioma (`apps/web/public/models/biomes/`, gerados por `pnpm models:biomes`) são espelhados por diretório inteiro — nenhuma criatura os referencia; quem os consome é a cena do mapa. Eles ficam como `.gltf` + texturas compartilhadas de propósito: empacotar em `.glb` embutiria uma cópia privada da textura da casca em cada árvore. O kit de personagens humanos (`apps/web/public/models/characters/`, gerado por `pnpm models:characters`) segue o mesmo contrato de espelhamento por diretório.

**Arte de card por criatura segue o mesmo contrato de espelhamento, sem coluna nova em `creatures`.** `apps/web/public/crt-cards/CRT-XXX.png` — um `.png` por criatura, nomeado pelo código exato — é a arte que a listagem (`CardImage` em `Bestiary.tsx`), a ficha (`CreatureDetail.tsx`) e o jogo (`CreatureInfoPanel`, `ActiveCreaturePanel`) mostram. O vínculo é só o nome do arquivo bater com `code`; não existe `cardUrl` no banco porque não há nada para resolver além do próprio código, ao contrário de `modelUrl` (que precisa apontar para um placeholder N:1). Cobertura parcial é o estado normal, não erro — hoje 21 das 60 criaturas têm card, e tanto `CardImage` quanto os dois painéis do jogo escondem a imagem em vez de mostrar quebrada quando falta. `pnpm game:export` espelha o diretório inteiro para `<repo-godot>/cards/`, mesmo contrato de `models/biomes` e `models/characters` — nenhuma criatura referencia o próprio card por URL no bundle.

**`cardPalette` ainda sai no bundle, mas sem leitor do lado do jogo desde 2026-09.** `export-game-data.mjs` continua extraindo shadow/mid/highlight de cada card (k-means com k=3 sobre os pixels OPACOS do PNG, via `sharp`) e escrevendo em `creature.cardPalette` — mesmo mecanismo de sempre, semente DETERMINÍSTICA (pontos espaçados uniformemente na lista de pixels, nunca sorteados) pra dois exports do MESMO card saírem com a MESMA paleta. O que mudou: o repo Godot removeu a recoloração de corpo por elemento/carta inteira (o elenco convergiu pra um único placeholder genérico, e recolorir por elemento parou de distinguir qualquer coisa — ver `../avyron/CLAUDE.md`), então `ElementPalette` lá não lê mais `palette` nem `cardPalette`. Isso deixa `cardPalette` um campo sem consumidor — contraria a regra deste arquivo sobre "campo do bundle é promessa: ou alguém lê, ou ele não viaja" (seção "Onde cada informação mora"). Não removido nesta rodada de propósito: é decisão de conteúdo do bestiário (parar de computar a extração, ou achar um novo uso), fora do escopo de "lógica dos objetos 3D" que motivou a limpeza do lado do jogo. Criatura sem card continua recebendo `cardPalette: null` — sem mudança nesse contrato.

**Corpo Meshy AI definitivo: `pnpm models:meshy` normaliza os clipes pro vocabulário canônico.** O Meshy libera baixar num `.glb` ÚNICO (malha + esqueleto + todos os clipes já dentro) — **essa é a forma PREFERIDA pra todo export novo**, e tudo que `scripts/convert-meshy.mjs --source <arquivo>.glb --out apps/web/public/models/<CODE>.glb` faz nesse caso é renomear os clipes. O caminho multi-arquivo (`--source <pasta>/`) existe porque o PILOTO foi baixado assim antes de sabermos da opção de arquivo único: o Meshy AI exportou um `<Nome>_Character_output.glb` (malha + esqueleto + um clipe de bind-pose inútil) e um `<Nome>_Animation_<Clipe>_withSkin.glb` POR ANIMAÇÃO — cada um com sua própria cópia da malha/esqueleto inteiros, só pra carregar um clipe. Pra esse caso o script funde os N arquivos num só: usa `@gltf-transform/functions.copyToDocument` com um `resolve` customizado que, ao encontrar um `Node` cujo NOME já existe no documento-base (o esqueleto é idêntico em todo arquivo do mesmo export), devolve o nó do BASE em vez de clonar — as animações passam a apontar pro esqueleto que a malha já usa, sem arrastar uma segunda cópia por clipe. Continua no script porque o piloto já integrado depende dele; export novo não deveria precisar. Nome de clipe vem de dentro de `Armature|<Clipe>|baselayer` (convenção do Blender), mapeado pro vocabulário canônico por `MESHY_CLIP_MAP` — tabela própria do Meshy, dialeto diferente do `CLIP_MAP` de `convert-placeholders.mjs`. Resultado: `<CODE>.glb` solto em `apps/web/public/models/`, MESMA convenção do Meshy legado — 1:1 com uma criatura (nunca N:1 como os placeholders), `syncModels` liga o `modelUrl` sozinho pelo nome do arquivo. `pnpm models:optimize` continua obrigatório depois (nenhum `.glb` do Meshy serve sem KTX2). Sem recoloração por elemento nem por carta em nenhum corpo Meshy — a arte já vem pronta, e o jogo parou de remapear atlas em 2026-09 (ver `../avyron/CLAUDE.md`).

**Piloto (2026-09): CRT-002 (Anomalocaris) recebeu o primeiro corpo Meshy AI animado.** Chegou primeiro como export multi-arquivo (12 `.glb`, um por clipe — o que motivou o caminho de fusão em `convert-meshy.mjs`), depois foi trocado pelo `.glb` único equivalente assim que soubemos que o Meshy libera essa opção — o caminho preferido hoje, ver o bullet acima. Na troca, `Charged_Spell_Cast_1` (mapeado pra `Attack2`) saiu do export por engano, junto com um clipe de verdade sem uso — quem exportou confundiu os dois. `Rising_Flying_Kick`, que antes ficava sem mapeamento (nenhum canônico livre), preencheu a vaga de `Attack2` — ver o comentário de `MESHY_CLIP_MAP` em `convert-meshy.mjs`. Cobertura final: todos os 11 clipes do arquivo mapeados, nenhum sobra sem uso. O vínculo inicial tinha ido pro CRT-010 por engano (escolha da sessão que integrou o piloto, sem confirmar com quem está autorando o conteúdo) — corrigido pra CRT-002, e a política daqui pra frente é quem faz a integração NÃO adivinhar a espécie, só perguntar. Substituiu o vínculo anterior do CRT-002 com `dungeon/Imp` — upgrade de placeholder genérico pra corpo definitivo, exatamente o que o ROADMAP do jogo já previa ("os modelos definitivos substituem placeholder por placeholder via `modelUrl`, sem tocar em código"). Um bug real do Godot apareceu na integração e foi corrigido do lado do jogo (não deste script): malha SKINADA media AABB ~100x maior que o alvo porque `CreatureActor._local_aabb` compunha a escala do nó "Armature" (0,01 — correção cm→m do Blender/Mixamo) por cima de uma escala que o import do Godot já bakeia nas poses dos ossos (`apply_root_scale`), contando a mesma correção duas vezes. Ver o comentário do ramo `if mi.skeleton != NodePath()` em `creature_actor.gd` no repo do jogo — vale pra QUALQUER corpo Meshy AI animado futuro que chegar com esqueleto não-Blender-nativo (a maioria: Mixamo, o esqueleto de fato padrão do mercado, carrega essa correção quase sempre).

**Clipes se transplantam entre corpos Meshy: `pnpm models:transfer` (2026-09-07).** O Meshy auto-riga todo bípede no MESMO esqueleto de 24 ossos (nomes, hierarquia, raiz `Hips`), mas cada modelo sai com os clipes que quem exportou marcou na hora — onze dos catorze do PZ-01 chegaram só com `Walk`/`Run`, e o CRT-005 com o conjunto inteiro (`Idle`, `Attack`/`Attack2`/`Attack3`, `HitReact`, `Death`, `Swim`, `Swim_Idle`). `scripts/transfer-clips.mjs --from <doador> --to <receptor>...` copia os clipes que o receptor NÃO tem, retargetados: copiar cru não serve, porque a rotação de REPOUSO de cada osso varia com as proporções da malha (até 146° num `Spine02`) e um quadro-chave é rotação local absoluta — medido, cru desvia 9°–33° por osso em média. O que se transfere é o desvio do repouso no espaço do osso-pai (`rest_receptor · rest_doador⁻¹ · q_doador`), com o `Hips` escalado pela razão das alturas de repouso e os demais ossos mantendo o comprimento do receptor. A prova é o `--verify`: `Walk`/`Run` estão dos dois lados de todo par, então o script retargeta o do doador e mede contra o que o receptor já tem — 3°–6° de média, 12°–30° no pior osso, contra 50°+ na ordem inversa. Por isso os clipes que o receptor já tem nunca são substituídos: são o gabarito. Roda sobre `.glb` já otimizados (o round-trip preserva o KTX2 byte a byte), guarda o original em `apps/web/.model-backups/pre-transfer/` e o passo seguinte é `pnpm game:export`. Foi assim que os onze ganharam o conjunto do CRT-005 em 2026-09-07; um export Meshy novo com os clipes de verdade continua sendo melhor que o transplante, que herda os gestos de outro corpo. **Herda também o root motion do doador** — e os catorze corpos de criatura foram convertidos em 2026-09-06, um dia ANTES de `convert-meshy.mjs` passar a remover a viagem do osso raiz: `Swim`, `Attack2` e `Death` andavam 1,0–1,7 m por ciclo, e no jogo a companheira "nadava mais rápido e resetava" a cada volta do clipe. A remoção vive em `scripts/lib/root-motion.mjs` (uma só, para os três caminhos), o transplante a aplica nos clipes que copia, e `pnpm models:strip <arquivos>` corrige in-place um `.glb` já convertido (idempotente, original em `.model-backups/pre-strip/`). `test_meshy_bodies.gd` no jogo mede o quadril de todo clipe de todo corpo do PZ-01 e reprova se algum andar mais de 5 cm.

**Placeholders "sem clipe" existem, e não são um bug do export.** `pnpm models:dungeon` (`scripts/convert-dungeon-monsters.mjs`) copia corpos do kit "Bestiary - Dungeon Monsters" (Quaternius, CC0) pro mesmo `apps/web/public/models/placeholders/<grupo>/` de sempre, mas esse kit não traz animação embutida — ele roda no MESMO esqueleto da Universal Animation Library que o kit de personagens usa, e espera ser montado por retarget, não por clipe bakeado. `clips` no `manifest.json` desses corpos é informativo (documenta o vocabulário que o retarget disponibiliza do lado do jogo), não uma leitura do arquivo. O retarget em si é decisão de engine e vive só no repo do jogo (`CreatureActor._build_retargeted_animation`); aqui a única responsabilidade é servir o arquivo e o vínculo `modelUrl` como qualquer outro placeholder.

**Placeholders por família de elemento (`big`/`flying`/`dungeon`/`easyanimated`) foram retirados do repo Godot em 2026-09** — o elenco convergiu pra um único corpo genérico (`dungeon/Imp.glb`) pra toda criatura sem modelo definitivo, e a recoloração por elemento que os distinguia saiu junto (ver `../avyron/CLAUDE.md`). `pnpm models:placeholders` e os scripts de conversão (`convert-placeholders.mjs`, `convert-dungeon-monsters.mjs`) continuam existindo aqui — não foram desfeitos — mas o `avyron/scripts/dev/convert_easy_pack.gd` que fazia a ponte `.fbx → .glb` do Easy Animated Pack (Quaternius, CC0 — Rã/Aranha/Rato/Cobra/Vespa) foi apagado do lado do jogo junto com o resto do pack, então esse pack específico não tem mais como entrar sem recriar a ponte. Corpo Meshy AI DEFINITIVO (1:1, animado, sem recoloração) é o caminho pra frente — ver `scripts/convert-meshy.mjs` e a seção abaixo.

**`pnpm models:placeholders` mescla o `manifest.json`, não sobrescreve — e isso já foi bug uma vez.** `placeholder_models/` é local-only por grupo (cada dev traz sua cópia; nada aqui garante que TODOS os grupos existem na MESMA máquina ao mesmo tempo). Rodar o script com só um grupo presente (o caso comum, não a exceção) e escrever o manifest inteiro por cima destruiria os grupos ausentes localmente — foi exatamente o que aconteceu ao converter EasyAnimated numa máquina sem `Big/Flying/Quadruped/` presentes: 44 entradas viraram 6, e só sobrou porque nada tinha sido commitado ainda. O script hoje lê o `manifest.json` existente e substitui só as entradas dos grupos que a rodada atual realmente escaneou (`scannedGroups`), preservando o resto. Mexer nessa lógica sem manter a mesclagem reabre o mesmo buraco.
...
- **`modelUrl` é N:1 com placeholders compartilhados.** Enquanto não existem modelos definitivos animados, várias criaturas apontam para o mesmo `.glb` em `apps/web/public/models/placeholders/<grupo>/` (gerados de `placeholder_models/` por `pnpm models:placeholders`, packs CC0 do Quaternius; o script também emite o `manifest.json` que descreve cada corpo). O vínculo se faz por `PATCH /creatures/{code}` com `modelUrl` (changelog e versão automáticos) — o botão "vincular/alterar modelo" e o viewer 3D da ficha foram removidos do bestiário em 2026-09, junto com three.js; o bestiário não renderiza modelo, só serve o arquivo e o vínculo. O `syncModels` só gerencia URLs no padrão `/models/CRT-XXX.glb`: um arquivo definitivo que aparecer ganha do placeholder, mas ponteiro de placeholder nunca é apagado pela varredura — "nenhuma mudança" no botão de sincronizar é o estado normal com placeholders vinculados. Os clipes de animação são normalizados na conversão para um vocabulário único (`Idle`, `Walk`, `Run`, `Attack`, `Attack2`, `HitReact`, `Death`…) — o jogo nunca vê os nomes originais dos packs.
...
- **Não servir `.glb` do Meshy sem passar por `pnpm models:optimize`.** O arquivo cru custa ~89 MB de VRAM por criatura, e o `KHR_texture_basisu` que o script marca como obrigatório é o que o import do Godot espera. Ver [docs/MODEL_OPTIMIZATION.md](docs/MODEL_OPTIMIZATION.md).
...
- **`scripts/convert-meshy.mjs`** — funde o export multi-arquivo do Meshy AI (um `.glb` por clipe) num corpo animado só, pro pipeline `models:optimize` de sempre
- **`scripts/transfer-clips.mjs`** — transplanta clipes entre corpos Meshy do mesmo rig por retarget relativo ao repouso, com `--verify` medindo o desvio contra os clipes que o receptor já tem
- **`scripts/lib/root-motion.mjs`** — a remoção de root motion que `convert-meshy.mjs`, `transfer-clips.mjs` e `strip-root-motion.mjs` dividem; **todo clipe do jogo é in-place**, e uma cópia por script já divergiu uma vez
