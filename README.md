...
- **`packages/db`** — schemas Drizzle, migrations e o **snapshot** do conteúdo. O seed está **congelado** (bootstrap offline apenas) — conteúdo novo entra pela API, e uma máquina se hidrata com `pnpm db:restore`.
...
# 5. carrega o conteúdo versionado no repo — sem rede
pnpm db:restore
...
**Corpo definitivo** (1:1 com a criatura, sem recoloração) vem do fluxo "base + casca": casca gerada no Tripo Studio sobre a silhueta da mestre e vestida no esqueleto dela por `pnpm models:publish -- --code CRT-XXX`, que grava `<CODE>.glb` em `apps/web/public/models/` (o `syncModels` liga o `modelUrl` pelo nome do arquivo) e espelha no jogo. O arquivo não carrega animação; o jogo dá a biblioteca UAL por retarget. Checklist em `../mestre/README.md`, contrato em `docs/MODEL_OPTIMIZATION.md`, racional em `CLAUDE.md`.
